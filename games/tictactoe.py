...
class Game(AbstractGame):
    ...
    def step(self, action):
        ...
        return observation, reward * 20, done, bool_win
...
class TicTacToe:
    ...
    def step(self, action):
        ...
        #reward = 1 if self.have_winner() else 0 #originally sparse reward

        reward = self.reward_function(row, col)

        self.player *= -1

        return self.get_observation(), reward, done, bool_win #also return if we have won or not per game
    ...
    def reward_function(self, row, col): #row, col represents the action taken by muZero player 
        reward = 0 #if you don't do any of the below, then reward = 0

        #if there is winner, reward = 1
        if(self.have_winner()):
            reward = 1
        else:      #if you've blocked an opponent's win, reward = 0.5
            reward = self.reward_blocking(row, col)
        return reward
          
    def reward_blocking(self, row, col):
        #defensive play-- intermediate reward of 0.5 for blocking opponent wins
        #diagnoal block check
        reward = 0
        if(row == 2):
            if(col == 0):
                if(self.board[1,1] == -1*self.player and self.board[0,2] == -1*self.player):
                    reward = 0.5
            elif(col == 2):
                if(self.board[1,1] == -1*self.player and self.board[0,0] == -1*self.player):
                    reward = 0.5 
        elif(row == 1 and col ==1):
            if(self.board[2,0] == -1*self.player and self.board[0,2] == -1*self.player):
                reward = 0.5
            elif(self.board[0,0] == -1*self.player and self.board[2,2] == -1*self.player):
                reward = 0.5
        elif(row==0):
            if(col == 0):
                if(self.board[1,1] == -1*self.player and self.board[2,2] == -1*self.player):
                    reward = 0.5
            elif(col == 2):
                if(self.board[1,1] == -1*self.player and self.board[2,0] == -1*self.player):
                    reward = 0.5
        #Horizontal block check
        arr_player = self.board[row,:][numpy.where(self.board[row,:] == self.player)]
        arr_opponent = self.board[numpy.where(self.board[row,:] == -1*self.player)]
        if(len(arr_player) == 1 and len(arr_opponent) == 2):
            reward = 0.5
        #Vertical block check 
        arr_player =self.board[numpy.where(self.board[:,col] == self.player)]
        arr_opponent = self.board[numpy.where(self.board[:, col] == -1*self.player)]
        if(len(arr_player) == 1 and len(arr_opponent) == 2):
            reward = 0.5     
        return reward
